...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 365 ;	// stake age of full weight: 10 y
unsigned int nStakeTargetSpacing = 5 * 60;			// 60 sec block spacing

int64 nChainStartTime = 1478445890;
int nCoinbaseMaturity = 5;             //确认数
...
const string strMessageMagic = "WZpoint Signed Message:\n";
...
// const int MONTHLY_BLOCKCOUNT =  30 * 24 * 60;	    //
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in WZpoint
    bool fStrictPayToScriptHash = true; // Always active in WZpoint
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "WZpoint", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "06/11/2016, beijing, Bitcoin";
        CTransaction txNew;
        ...
        block.nNonce   = !fTestNet ? 867299 : 4365;

		///////////////////////////////////////////////////////////////////////////
        
		uint256 hashGenesisNew = block.GetHash();
        while (hashGenesisNew > bnProofOfWorkLimit.getuint256()){
            if (++block.nNonce==0) break;
            hashGenesisNew = block.GetHash();
		...
		if(hashGenesisNew <= bnProofOfWorkLimit.getuint256())
		{
			if (fTestNet)
            {
        ...
        assert(block.hashMerkleRoot == uint256("0xa8e5b3030d3af98bfeb3e810c352f1ad72176a30f8536a04c5ff59fed11ff194"));
...
unsigned char pchMessageStart[4] = { 0xae, 0xf2, 0x3a, 0xd5 };
